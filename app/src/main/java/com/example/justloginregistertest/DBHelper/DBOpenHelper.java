...
public class DBOpenHelper extends SQLiteOpenHelper {
     ...
     */
    public DBOpenHelper(Context context){//此处设置库名为db_user
        super(context,"db_user.db",null,1);
     ...
     */
    @Override
    public void onCreate(SQLiteDatabase db){//此处设置表名为user
        db.execSQL("CREATE TABLE IF NOT EXISTS user(" +
                "_id INTEGER PRIMARY KEY AUTOINCREMENT," +
                "name TEXT," +
                "password TEXT,"+
                "create_time DATETIME,"+
                "update_time DATETIME)");//规范数据库，id主键、创建时间、更新时间必需
    }
    @Override
